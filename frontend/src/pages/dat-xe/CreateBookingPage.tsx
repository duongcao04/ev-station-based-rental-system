...
export default function CreateBookingPage() {
  ...
  const { data: pickupStationsData, isLoading: isLoadingPickupStations } =
    useQuery({
  ...
  const { data: returnStationsData, isLoading: isLoadingReturnStations } =
    useQuery({
    ...
    ? (Array.isArray(pickupStationsData) ? pickupStationsData : []).map(
        (station: any) => ({
          ...
          name:
            ...
            station.address ||
            `Station ${(station.station_id || station.user_id)?.substring(
              0,
              8
            )}`,
  ...
  const createBookingMutation = useMutation({
    ...
    onError: (error: any) => {
      ...
      alert(
        error?.response?.data?.error || 'Đặt xe thất bại. Vui lòng thử lại.'
      );
    },
  });
  ...
  const handleSubmit = async () => {
    ...
    const selectedStartStation = pickupStations.find(
      (s) => s.id === startStation
    );
    ...
    const startStationId =
      selectedStartStation?.station_id ||
      selectedStartStation?.id ||
      startStation;
    const endStationId =
      selectedEndStation?.station_id || selectedEndStation?.id || endStation;
        ...
        <div className='mb-8 flex items-center gap-4'>
          ...
          <div className='flex-1'>
            ...
            <p className='text-gray-600 mt-1'>
              Hoàn thành thông tin đặt xe của bạn
            </p>
          </div>
        </div>
